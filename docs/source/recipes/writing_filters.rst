...
and when Reactor starts, it will download the dashboard schema from Elasticsearch and use the filters from that.
However, if the dashboard name changes or if there is connectivity problems when Reactor starts, the rule will not load and
Reactor will exit with an error like "Could not download filters for .."

The second way is to generate a config file once using the Kibana dashboard. To do this, run ``reactor-rule-from-kibana``.

.. code-block:: console

    $ reactor-rule-from-kibana
